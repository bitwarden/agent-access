...
The ´reach´ CLI tool has built-in support for connecting to the Bitwarden CLI. The interactive CLI can be used to unlock your vault (`/bw-unlock`) and create a pairing token that the remote side can use to connect.
...
reach connect --token <pairing-token> --output json
...
reach connect --token <pairing-token> --domain example.com --output json
