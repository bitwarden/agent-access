...
echo "Cross-compiling ap-proxy for ${TARGET}..."
cross build --release -p ap-proxy --target "$TARGET"

BINARY="$REPO_ROOT/target/${TARGET}/release/ap-proxy"
if [[ ! -f "$BINARY" ]]; then
