...
usage() {
    ...
    echo ""
    echo "Build the ap-proxy Docker image."
    echo ""
    echo "Options:"
    echo "  --name NAME        Image name (default: ap-proxy)"
...
PROXY_DIR="$REPO_ROOT/crates/ap-proxy"
...
# but we only include the crates ap-proxy actually depends on and trim the
...
assemble_context() {
    ...
    awk '
        ...
        in_members && /\]/ {
            print "members = ["
            print "    \"crates/ap-error\","
            print "    \"crates/ap-error-macro\","
            print "    \"crates/ap-proxy\","
            print "    \"crates/ap-proxy-client\","
            print "    \"crates/ap-proxy-protocol\","
            print "]"
            in_members=0
            next
        }
    ...
    # Copy only the crates ap-proxy depends on
    mkdir -p "$ctx/crates"
    for crate in ap-proxy ap-proxy-client ap-proxy-protocol ap-error ap-error-macro; do
